...
int main(int argc, char* argv[])
{
    ...
    while(st != S_END)
    {
        ...
        switch(st)
        {
            case S_WAIT:        // 此时只需将参数起始指针前移
                ++arg_beg;
                break;
            case S_ARG_END:     // 参数结束，将参数地址存入x_argv数组
                x_argv[arg_cnt++] = &lines[arg_beg];
                arg_beg = arg_end;
                *p = '\0';      // 替换为字符串结束符
                break;
            case S_ARG_LINE_END:    // 将参数地址存入x_argv数组中，同时执行指令
                x_argv[arg_cnt++] = &lines[arg_beg];    // 不加break，后续处理同S_LINE_END
            case S_LINE_END:        // 行结束，执行当前行指令
                arg_beg = arg_end;
                *p = '\0';
                if(fork() == 0)
                {
